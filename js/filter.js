...
function isiEmailTujuan() {
    ...
    // const hasilEmail = targetEmailProvider.value = emailTujuan;

    if (emailTujuan == "Icon Plus") {
        targetEmailProvider.value = "cs@iconpln.co.id";
        targetCcProvider.value = "angelin.filanda@iconpln.co.id, noc@jsn.net.id, ramadhan.yondra@iconpln.co.id";
        ...
        targetCcProvider.value = "nico.enjelit@indosatooredoo.com, noc@jsn.net.id,";
        ...
        targetCcProvider.value = "hari.pujiharto@fiberstar.co.id, noc@jsn.net.id,";
        targetSubjectEmail.value = issueMetro + " || JARINGANKU SARANA NUSANTARA || " + cidMetro;

    }else if (emailTujuan == "XL Axiata") {
        targetEmailProvider.value = "cs-busol@xl.co.id";
        targetCcProvider.value = "asria@xl.co.id, noc@jsn.net.id,";
        targetSubjectEmail.value = issueMetro + " || JARINGANKU SARANA NUSANTARA || " + cidMetro;

    }else if (emailTujuan == "H3I") {
        targetEmailProvider.value = "3corporatecare@three.co.id";
        targetCcProvider.value = "reno.huntoro@three.co.id, noc@jsn.net.id,";
        targetSubjectEmail.value = issueMetro + " || JARINGANKU SARANA NUSANTARA || " + cidMetro;

    }else if (emailTujuan == "Moratel") {
        targetEmailProvider.value = "cs@moratelindo.co.id";
        targetCcProvider.value = "hermawati@moratelindo.co.id, noc@jsn.net.id, ibnu.rachmadi@jsn.net.id";
        targetSubjectEmail.value = "JSN Site ( " + daerah + " )" + " - " + issueMetro ;
